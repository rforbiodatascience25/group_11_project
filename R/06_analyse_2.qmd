...
ri_annotation = read.csv("../data/03_ri_annotation_augment_data.csv")
```

## Calculate Correlation Matrix and fill NAs with 0
...
cluster_kos <- clusters_df |>
  left_join(ri_annotation, by = "gene_ids") |>
  drop_na(ko)
```

## Retrieve KEGG pathway for each KO term

```{r}
unique_ko_ids <- unique(gsub("ko:", "", ri_annotation$ko))
...
ko_pathway_df <- 
    bind_rows(lapply(names(ko_to_pathway_list), function(ko_id) {
        data.frame(
            ...
            pathway_name_raw = ko_to_pathway_list[[ko_id]]
        )
    })) |>
    mutate(
...
df_with_pathways <- cluster_kos |>
    left_join(ko_pathway_df, by = "ko")
```

## Calculate percentages of each pathway across clusters
...
## Filter and plot the 4 pathways with highest percentage
